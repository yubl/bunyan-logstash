...
LogstashStream.prototype.send = function logstashSend(message) {
  ...
  self.requested += 1;
  self.client.send(buf, 0, buf.length, self.port, self.host, function () { self.sent += 1; });
};

LogstashStream.prototype.close = function (callback) {
  var pending = () => this.requested !== this.sent;
